...
const Sidebar = () => {
  ...
  return (
    <>
      <div className={`${enabled ? 'bg-[#2b2c3b] border-r-gray-700' : 'border-r-gray-300  bg-white'} hidden sm:${hiddenSidebar ? 'hidden' : 'flex'} flex-col justify-between items-start w-[23%] h-full border-r-1 py-4`}>
        <div className='flex flex-col justify-start items-center h-full overflow-hidden w-full gap-3'>
          <p className='text-start w-9/10 px-7 text-[#828FA3] font-semibold py-2'>All Boards ({boards.length})</p>
        ...
        <div className='flex flex-col items-center justify-center w-full'>
          ...
          <div className='flex flex-row justify-start items-center cursor-pointer hover:bg-[#EEF1F8] text-[#828FA3] w-9/10  px-7 gap-2 py-4 rounded-r-2xl '
            onClick={() => sethiddenSidebar(true)}>
            <EyeOff size={19} />
            <p className='text-[17px] font-semibold'>Hide Sidebar</p>
          </div>
        </div>
      </div>
